...
import { caseRadius } from '@/lib/design-tokens';
  ...
  return (
    <div style={{ marginTop: '52px' }}>
      <div style={{ display: 'flex', marginBottom: '12px' }}>
        <div style={{ background: '#FFFFFF', border: '1px solid rgba(10,10,10,0.08)', borderRadius: caseRadius.full, display: 'inline-flex', gap: '5px', padding: '4px' }}>
          {[
            ['ranking', 'Ranking MVP'],
            ...
            <button
              ...
              style={{ background: mode === key ? blue : 'transparent', border: 0, borderRadius: caseRadius.full, color: mode === key ? '#FFFFFF' : 'rgba(10,10,10,0.66)', cursor: 'pointer', fontSize: '12px', fontWeight: 500, padding: '7px 13px' }}
            >
              {label}
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-[minmax(0,3.62fr)_minmax(0,1fr)]">
        <div style={{ alignItems: 'center', aspectRatio: '189 / 107', background: '#FFFFFF', border: '8px solid #E5E7EB', borderRadius: caseRadius.xl, display: 'flex', justifyContent: 'center', overflow: 'hidden' }}>
          <Image src={mode === 'outreach' ? '/img/connectnova/DashboardLayout.avif' : '/img/connectnova/Dashboard.avif'} alt={mode === 'outreach' ? 'ConnectNova platform with outreach' : 'ConnectNova ranking MVP'} width={1512} height={856} sizes="(max-width: 768px) 100vw, 80vw" style={{ height: '100%', objectFit: 'cover', width: '100%' }} />
        </div>
        <div style={{ alignItems: 'center', aspectRatio: '136 / 279', background: '#FFFFFF', border: '8px solid #E5E7EB', borderRadius: caseRadius.xl, display: 'flex', justifyContent: 'center', overflow: 'hidden' }}>
          <Image src="/img/connectnova/Extension_home.avif" alt="ConnectNova Chrome extension" width={816} height={1674} sizes="(max-width: 768px) 100vw, 200px" style={{ height: '100%', objectFit: 'contain', width: '100%' }} />
        </div>
      </div>
    </div>
  );
...
function ExistingImage({ src, alt, contain = false, frameless = false }: { src: string; alt: string; contain?: boolean; frameless?: boolean }) {
  return (
    <div style={{ alignItems: 'center', background: frameless ? 'transparent' : '#FFFFFF', border: frameless ? 0 : '1px solid rgba(10,10,10,0.1)', borderRadius: frameless ? 0 : caseRadius.lg, display: 'flex', justifyContent: 'center', overflow: 'hidden', padding: frameless ? 0 : '12px' }}>
...
function DesignFoundationVisual() {
  ...
  return (
    <div style={{ background: '#FFFFFF', border: '1px solid rgba(10,10,10,0.1)', borderRadius: caseRadius.lg, overflow: 'hidden' }}>
      <div style={{ alignItems: 'center', background: 'rgba(10,10,10,0.015)', borderBottom: '1px solid rgba(10,10,10,0.07)', display: 'flex', flexWrap: 'wrap', gap: '14px', justifyContent: 'space-between', padding: '16px 24px' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>{['Color', 'Typography', 'Spacing', 'Radius'].map((item) => <span key={item} style={{ background: 'rgba(10,10,10,0.05)', borderRadius: caseRadius.sm, color: 'rgba(10,10,10,0.46)', fontSize: '10px', padding: '3px 8px' }}>{item}</span>)}</div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4">
        <div className="border-b border-black/10 p-6 md:border-r xl:border-b-0">
          <p style={columnHeading}>Color</p>
          {colors.map(([name, value, color]) => <div key={name} style={{ alignItems: 'center', display: 'flex', gap: '9px', marginBottom: '9px' }}><span style={{ background: color, border: '1px solid rgba(10,10,10,0.1)', borderRadius: caseRadius.sm, flexShrink: 0, height: '22px', width: '22px' }} /><span style={{ minWidth: 0 }}><strong style={{ color: ink, display: 'block', fontSize: '10px', fontWeight: 600 }}>{name}</strong><small style={{ color: 'rgba(10,10,10,0.38)', display: 'block', fontSize: '8px' }}>{value}</small></span></div>)}
        </div>
  ...
  return (
    <>
      <Section label="Project at a glance">
        ...
        <p style={{ color: ink, fontSize: 'clamp(20px, 2.2vw, 30px)', fontWeight: 400, letterSpacing: '-0.015em', lineHeight: 1.35, margin: '54px 0 0', maxWidth: '980px' }}>Source finds the right people. Qualify helps users decide. Engage starts the conversation. Manage preserves progress toward an outcome.</p>
      </Section>
      ...
      <Section label="The Challenge" tone="soft">
        ...
        <p style={{ color: ink, fontSize: 'clamp(20px, 2.2vw, 30px)', fontWeight: 400, letterSpacing: '-0.015em', lineHeight: 1.35, margin: 'clamp(64px, 8vw, 96px) 0 0', maxWidth: '920px' }}>The challenge was turning hundreds of scattered profiles into a <span style={{ color: blue }}>clear, prioritized decision.</span></p>
      </Section>
      ...
      <Section label="The Pivotal Product Decision">
        ...
        <div style={{ background: '#EDF3FF', borderRadius: caseRadius.lg, marginTop: 'clamp(64px,8vw,96px)', overflow: 'hidden', padding: 'clamp(38px,6vw,68px)' }}><Eyebrow>Core insight</Eyebrow><h2 style={{ color: ink, fontSize: 'clamp(26px,3.4vw,40px)', fontWeight: 400, letterSpacing: '-0.025em', lineHeight: 1.18, margin: '0 0 52px', maxWidth: '920px' }}>A profile only became meaningful within a specific goal.</h2><p style={{ color: muted, fontSize: '16px', lineHeight: 1.65, margin: '0 0 40px', maxWidth: '840px' }}>The same person could be highly relevant to one hiring brief or sales target and irrelevant to another. Saving a profile alone could not explain why the person mattered or how they should be evaluated.</p><div className="case-radius-lg overflow-hidden grid grid-cols-1 gap-px bg-blue-200/60 md:grid-cols-5">{projectQuestions.map((question) => <p key={question} style={{ background: '#F8FAFF', color: ink, fontSize: '13px', lineHeight: 1.5, margin: 0, minHeight: '130px', padding: '22px' }}>{question}</p>)}</div></div>
        <p style={{ color: ink, fontSize: 'clamp(20px, 2.2vw, 30px)', fontWeight: 400, letterSpacing: '-0.015em', lineHeight: 1.35, margin: 'clamp(64px, 8vw, 96px) 0 0', maxWidth: '920px' }}>Project became the bridge between collecting a profile and <span style={{ color: blue }}>making a decision about it.</span></p>
      </Section>
      ...
      <Section label="Information Architecture">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2" style={{ marginTop: '64px' }}><div style={{ background: '#F3F4F6', borderRadius: caseRadius.lg, overflow: 'hidden', padding: '36px' }}><Eyebrow>Before</Eyebrow><div style={{ color: muted, fontSize: '18px', lineHeight: 2.1 }}>Project list<br />→ Profile list<br />→ Profile detail</div><p style={{ color: muted, fontSize: '12px', margin: '28px 0 0' }}>Three separate navigation layers</p></div><div style={{ background: '#EDF3FF', border: `1px solid ${blue}`, borderRadius: caseRadius.lg, overflow: 'hidden', padding: '36px' }}><Eyebrow>After</Eyebrow><div style={{ color: ink, fontSize: '18px', lineHeight: 2.1 }}>Project list<br />→ Project workspace</div><div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '26px' }}>{['People', 'Ranking', 'Evaluation', 'Profile detail'].map((item) => <span key={item} style={{ background: '#FFFFFF', border: '1px solid rgba(36,89,211,0.2)', borderRadius: caseRadius.sm, color: blue, fontSize: '11px', padding: '8px 10px' }}>{item}</span>)}</div></div></div>
        <div style={{ marginTop: '56px' }}><div style={{ borderRadius: caseRadius.xl, overflow: 'hidden' }}>{informationArchitectureVisual ?? <ExistingImage src="/img/connectnova/Dashboard.avif" alt="ConnectNova Project workspace" contain />}</div><div className="case-radius-lg overflow-hidden grid grid-cols-1 gap-px bg-black/10 md:grid-cols-4" style={{ marginTop: '18px' }}>{['Profiles organized by goal', 'Evaluation visible in context', 'Details available without leaving the workspace', 'Ranking and review in one place'].map((item) => <p key={item} style={{ background: '#F1F4F8', color: muted, fontSize: '12px', margin: 0, padding: '18px', textAlign: 'center' }}>{item}</p>)}</div></div>
        <p style={{ color: ink, fontSize: 'clamp(20px, 2.2vw, 30px)', fontWeight: 400, letterSpacing: '-0.015em', lineHeight: 1.35, margin: 'clamp(64px, 8vw, 96px) 0 0', maxWidth: '920px' }}>Users could move between overview and detail without losing the <span style={{ color: blue }}>Project context</span> behind each evaluation.</p>
      </Section>
      ...
      <Section label="Collect Without Leaving LinkedIn">
        ...
        <div style={{ background: '#EDF3FF', borderRadius: caseRadius.lg, marginTop: '58px', overflow: 'hidden', padding: 'clamp(24px,5vw,56px)' }}><h2 style={{ color: ink, fontSize: 'clamp(22px,2.7vw,34px)', fontWeight: 400, letterSpacing: '-0.015em', lineHeight: 1.2, margin: '0 0 32px' }}>Collect directly inside the existing workflow</h2><ExistingImage src="/img/connectnova/Extension.avif" alt="ConnectNova profile collection flow in the Chrome extension" contain frameless /><div className="case-radius-lg overflow-hidden grid grid-cols-1 gap-px bg-blue-200/60 md:grid-cols-3" style={{ marginTop: '18px' }}>{[
      ...
      <Section label="Editable AI Criteria" tone="soft">
        ...
        <p style={{ color: ink, fontSize: 'clamp(20px, 2.2vw, 30px)', fontWeight: 400, letterSpacing: '-0.015em', lineHeight: 1.35, margin: 'clamp(64px, 8vw, 96px) 0 0', maxWidth: '920px' }}>AI did not make the final decision. It helped users define a <span style={{ color: blue }}>clearer and more consistent way</span> to make it.</p>
      </Section>
      ...
      <Section label="Manage, Rank, Decide">
        ...
        <p style={{ color: ink, fontSize: 'clamp(20px, 2.2vw, 30px)', fontWeight: 400, letterSpacing: '-0.015em', lineHeight: 1.35, margin: 'clamp(64px, 8vw, 96px) 0 0', maxWidth: '920px' }}>The dashboard turned a collection of saved profiles into a <span style={{ color: blue }}>prioritized working list.</span></p>
      </Section>

      <Section label="Building and Shipping in Six Weeks" tone="soft" hidden>
        <Eyebrow>07 — Building and shipping in six weeks</Eyebrow><Title width="900px">Create enough structure to move fast without creating chaos.</Title><Body width="800px"><p>Within six weeks, the MVP had to connect a Chrome Extension and web dashboard as one coherent product.</p><p>I built a lightweight foundation of Figma Variables, semantic tokens, and reusable components to keep design and engineering aligned.</p></Body>
        <div style={{ background: '#FFFFFF', border: '1px solid rgba(10,10,10,0.1)', borderRadius: caseRadius.lg, marginTop: '58px', overflow: 'hidden', padding: 'clamp(30px,6vw,68px)' }}><h2 style={{ color: ink, fontSize: 'clamp(36px,4.8vw,60px)', fontWeight: 400, letterSpacing: '-0.03em', margin: '0 0 20px' }}>Build the system alongside the product.</h2><p style={{ color: muted, fontSize: '16px', lineHeight: 1.65, margin: '0 0 44px', maxWidth: '820px' }}>The goal was not to create a comprehensive design system. It was to establish the minimum structure required for consistent and efficient delivery.</p><div className="case-radius-lg overflow-hidden grid grid-cols-1 lg:grid-cols-2" style={{ border: '1px solid rgba(10,10,10,0.1)' }}><div style={{ background: '#F5F7FA', minHeight: '480px', padding: '36px' }}><Eyebrow>Figma Variables</Eyebrow>{['Color', 'Typography', 'Spacing', 'Radius', 'Component states'].map((item) => <div key={item} style={{ background: '#FFFFFF', border: '1px solid rgba(10,10,10,0.08)', borderRadius: caseRadius.md, color: ink, fontSize: '13px', marginTop: '10px', padding: '16px' }}>{item}</div>)}</div><div style={{ background: '#FFFFFF', borderLeft: '1px solid rgba(10,10,10,0.1)', minHeight: '480px', padding: '36px' }}><Eyebrow>Tokens and reusable components</Eyebrow><div className="grid grid-cols-2 gap-3">{['Semantic color', 'Spacing scale', 'Type scale', 'Corner radius', 'Buttons', 'Inputs', 'Navigation', 'Cards', 'Tables', 'Status states'].map((item) => <div key={item} style={{ background: '#F7F8FA', border: '1px solid rgba(10,10,10,0.07)', borderRadius: caseRadius.md, color: muted, fontSize: '11px', minHeight: '72px', padding: '16px' }}>{item}</div>)}</div></div></div><div className="case-radius-lg overflow-hidden grid grid-cols-1 gap-px bg-black/10 md:grid-cols-3" style={{ marginTop: '22px' }}>{[
        ...
        <p style={{ color: ink, fontSize: 'clamp(20px, 2.2vw, 30px)', fontWeight: 400, letterSpacing: '-0.015em', lineHeight: 1.35, margin: 'clamp(64px, 8vw, 96px) 0 0', maxWidth: '920px' }}>The design system was part of the MVP delivery strategy - <span style={{ color: blue }}>not a separate project.</span></p>
      </Section>
      ...
      <Section label="Building and Shipping in Six Weeks" tone="soft">
        ...
        <Body width="800px">
          <p>Within six weeks, the MVP had to connect a Chrome Extension and web dashboard as one coherent product.</p>
          <p>I built a lightweight foundation of Figma Variables, semantic tokens, and reusable components to keep design and engineering aligned.</p>
        </Body>
        <div style={{ marginTop: '58px' }}>
          <h2 style={{ color: ink, fontSize: 'clamp(24px,3vw,36px)', fontWeight: 400, letterSpacing: '-0.02em', lineHeight: 1.2, margin: '0 0 20px' }}>Build the system alongside the product.</h2>
          <p style={{ color: muted, fontSize: '16px', lineHeight: 1.65, margin: '0 0 44px', maxWidth: '760px' }}>The goal was the minimum structure needed for consistent delivery, not a comprehensive design system.</p>
          ...
          <div className="case-radius-lg overflow-hidden grid grid-cols-1 gap-px bg-black/10 md:grid-cols-3" style={{ marginTop: '22px' }}>
            {[
              ['Shared visual rules', 'Recurring decisions stayed visible and reusable.'],
              ['Reusable values', 'Design and engineering could work from consistent definitions.'],
              ['Consistent states', 'The Chrome Extension and web dashboard shared the same visual and interaction logic.'],
      ...
      <Section label="Results and Validation">
        <Eyebrow>08 — Results and validation</Eyebrow><Title>What the MVP validated—and what still needed work</Title><Body><p>The six-week MVP connected LinkedIn collection, Project-based organization, and AI-assisted evaluation into one workflow. It improved the core experience while showing where broader validation was still needed.</p></Body>
        <h2 style={{ color: ink, fontSize: 'clamp(24px,3vw,36px)', fontWeight: 400, letterSpacing: '-0.02em', lineHeight: 1.2, margin: '72px 0 34px' }}>A more effective core workflow</h2><div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4" style={{ borderBottom: '1px solid rgba(10,10,10,0.14)', borderTop: '1px solid rgba(10,10,10,0.14)' }}>{[
      ...
      <Section label="Beyond the Validated MVP" tone="soft">
        ...
        <div style={{ marginTop: '48px' }}><ExistingImage src="/img/connectnova/Workflow.avif" alt="ConnectNova outreach platform information architecture" contain frameless /></div>
        ...
        <div style={{ marginTop: 'clamp(64px,8vw,96px)' }}>
          ...
          <div style={{ background: '#FFFFFF', border: '1px solid rgba(10,10,10,0.12)', borderRadius: caseRadius.xl, height: 'min(620px,72svh)', overflow: 'hidden' }}>
            <iframe
        ...
        <p style={{ color: ink, fontSize: 'clamp(20px, 2.2vw, 30px)', fontWeight: 400, letterSpacing: '-0.015em', lineHeight: 1.35, margin: 'clamp(64px, 8vw, 96px) 0 0', maxWidth: '920px' }}>Outreach was the next extension of the workflow - <span style={{ color: blue }}>not part of the validated core MVP yet.</span></p>
      </Section>
